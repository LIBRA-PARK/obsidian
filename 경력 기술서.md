...
## 설계 목적
서비스 중인 'AnyLink 7' 프레임워크를 계승하는 신규 프레임워크 프로젝트인 'AnyLink 8'은 빠른 개발과 출시를 통해서 운영 중 프로토콜을 Add-on 하는 방식으로 기획했다. 최종적으로 'AnyLink 7'가 지원하는 'n'개의 프로토콜을 모두 구현하여 동일한 기능을 제공하며, 요구되는 신규 프로토콜에 대해서도 손 쉽게 확장할 수 있도록 한다.

인터페이스 프레임워크의 프로토콜은 연결 관리, 메세지 관리 등 다양한 옵션을 설정할 수 있도록 엔드포인트 개발이 필요하다. 엔드포인트는 설정된 프로토콜과 옵션에 맞춰 동작할 수 있도록 엔진에 배포되어 실행되는 주체가 되어 서비스를 제공한다. 모든 프로토콜은 엔드포인트에 서로 상이한 옵션이 설정되고 프로토콜별 특화 서비스가 추가로 제공되지만 C.R.U.D와 배포되는 서비스는 공통적이다. 
