...
하지만 양방향 연관관계 매핑을 사용으로 비즈니스 로직 내에 관계를 관리 해야 하는 코드가 추가되고 비즈니스 로직 코드의 가독성을 저하 시키는 현상과, 대중적인 문제였던 의도치 않는 쿼리 발생, N + 1 문제 등 발생되었다. 이 문제로 ORM 사용으로 Marshal / UnMarahal 을 제거 했음에도 관계 유지를 위한 코드가 추가되는 등 제자리 걸음이라고 판단되어 양방향 관계를 모두 제거하기 시작했다. 양방향 매핑을 사용하지 않고 단방향 매핑을 사용해 양 객체간 관계를 매핑하기 위한 복잡한 코드 최소화하고, 단순 관계만을 선언할 수 있도록 변경되어 비즈니스 로직을 작성할 때 한 쪽의 관계만을 주의하면 되기 때문에 유지보수성도 충분히 보장할 수 있겠다고 생각했다. 이에 대대적으로 양방향 매핑을 모두 삭제하고 
