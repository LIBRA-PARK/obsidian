...
인터페이스 프레임워크의 프로토콜은 연결 관리, 메세지 관리 등 다양한 옵션을 설정할 수 있도록 엔드포인트 개발이 필요하다. 엔드포인트는 설정된 프로토콜과 옵션에 맞춰 동작할 수 있도록 엔진에 배포되어 실행되는 주체가 되어 서비스를 제공한다. 모든 프로토콜은 엔드포인트에 서로 상이한 옵션이 설정되고 특정 서비스가 추가로 제공되지만 C.R.U.D와 배포되는 서비스는 공통적이다. 
## 문제 정의
